...
int negamax(int *board, int depth, int alpha, int beta, int color, struct LINE *pline){
	...
	if (depth == 0){
		pline ->cmove = 0;
		return (color == board[64] ? 1 : -1)  * scoreval(board) * (board[64] == WHITE ? 1 : -1);
	}
...
void clear_board(int *board){
	int i;
	for (i=0; i<65; ++i) board[i] = EMPTY;
}

void reset_board(int *board){
	clear_board(board);
	put_fen(board, "6k1/1p4pp/1qp5/2N1p3/1P3P2/1P1P3P/r2Q4/r4RK1 ");
...
void think(int *board, int depth, int color, struct LINE *line){
	...
	board[64] = color;
	//printf(" %d ", board[60]);
	int score = negamax(board, depth, -INFINITY, +INFINITY, color, line);
	//printf(" %d ", board[60]);
